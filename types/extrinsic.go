...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/JFJun/substrate-rpc-go/scale"
	"github.com/JFJun/substrate-rpc-go/signature"
...
func NewCall( callIdx string, args ...interface{}) (Call, error) {
	if len(callIdx) != 4 {
		return Call{}, fmt.Errorf("callIdx length is not equal 4,len=%d", len(callIdx))
	}
	...
	c := CallIndex{SectionIndex: m[0], MethodIndex: n[0]}
	var a []byte
